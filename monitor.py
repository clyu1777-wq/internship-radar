...
LEVEL = re.compile(
    ...
    r"|\bintern(ship)?s?\b|\bco[\s-]?ops?\b|university|new[\s-]?grad|graduate\s*program|early[\s-]?career|apprentic|student|trainee|\bresiden|campus",
    re.I)
...
INTERN_MARK = re.compile(r"实习|\bintern(ship)?s?\b|\bco[\s-]?ops?\b|日常实习", re.I)
...
def f_ashby(src):
    ...
    return [{"id": j["id"], "title": j["title"].strip(), "url": j.get("jobUrl", ""),
             ...
             "_track": "intern" if "intern" in str(j.get("employmentType", "")).lower() else None}
